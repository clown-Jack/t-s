        ...
        return (
            <>
                ...
                <Form onFinish={this.handleSubmit} onFinishFailed={this.failedSubmit}>
                    ...
                    >
                        <Input prefix={<MailOutlined />} style={{ color: "rgba(0,0,0,.25)" }} type="text" placeholder="请输入邮箱" />
                    </Form.Item>
                    <Form.Item name="phone"
                        rules={[{ required: true, message: '手机号必须输入' }]}
                    >
                        <Input prefix={<PhoneOutlined />} style={{ color: "rgba(0,0,0,.25)" }} type="text" placeholder="请输入手机号" />
                    </Form.Item>
                    <Form.Item>
                        <Button type="primary" htmlType="submit" className="login-form-button">注册</Button>
                        <span>或者</span><Link to="/login">立即登录</Link>
                    </Form.Item>
                </Form>
            </>
        )
